...
%code requires{
   ...
   extern map<std::string, std::string> predefined;
...
%code{
   ...
   int prev_param_offset = 8;
   int prev_var_offset = 0;
   map<std::string, vector<std::string>> gen_code;
   map<std::string, vector<std::string>> rodata;
   int line_no = 1;
...
%nterm <int> program translation_unit struct_specifier function_definition fun_declarator
...
%nterm <class statement_astnode*> statement assignment_statement procedure_call printf_call iteration_statement selection_statement
...
function_definition: 
     type_specifier fun_declarator compound_statement
     {
          ...
          gen_code[func_name].push_back("\taddl\t$" + to_string(tot_local_size) + ", %esp");
          gen_code[func_name].push_back("\tleave");
          gen_code[func_name].push_back("\tret");
          gen_code[func_name].push_back("\t.size\t" + func_name + ", .-" + func_name);
          lst = new class ST();
          prev_var_offset = 0;
          func_name = "";
          return_type = "";
...
main_definition:
     INT MAIN '(' ')' 
     {
          return_type = $1;
          func_name = $2;
          gen_code[func_name] = vector<std::string>();
          rodata[func_name] = vector<std::string>();
          gen_code[func_name].push_back("\t.globl\t" + func_name);
          gen_code[func_name].push_back("\t.type\t" + func_name + ", @function");
          gen_code[func_name].push_back(func_name + ":");
          gen_code[func_name].push_back("\tpushl\t%ebp");
          gen_code[func_name].push_back("\tmovl\t%esp, %ebp");
     }

     compound_statement
     {
          types = new class types_info($1);
          class Symbols* funcSym = new class Symbols($2, "fun", "global", 0, 0, $1, types); 
          gst -> addSymbol(funcSym);
          symtab_map[$2] = lst;
          gen_code[func_name].push_back("\taddl\t$" + to_string(tot_local_size) + ", %esp");
          gen_code[func_name].push_back("\tleave");
...
fun_declarator:
     ...
     { 
          ...
          gen_code[func_name].push_back("\t.globl\t" + func_name);
          gen_code[func_name].push_back("\t.type\t" + func_name + ", @function");
          gen_code[func_name].push_back($1 + ":");
          gen_code[func_name].push_back("\tpushl\t%ebp");
          gen_code[func_name].push_back("\tmovl\t%esp, %ebp");
          temp_st = gst -> get_elems();
...
compound_statement: 
     ...
     {
          ast_map[func_name] = $2;
          $2 -> gen(gen_code[func_name], rodata[func_name], gst -> get_elems(), lst -> get_elems(), func_name, return_type, symtab_map);
     }
     ...
     | '{' declaration_list '}'
     {
          ...
          gen_code[func_name].push_back("\tsubl\t$" + to_string(tot_local_size) + ", %esp");
          class nostatement* ns = new class nostatement();
          vector<class statement_astnode*> vec;
          vec.push_back(ns);    
          ast_map[func_name] = new class seq_astnode(vec);
     ...
     {
          ast_map[func_name] = $4;
          $4 -> gen(gen_code[func_name], rodata[func_name], gst -> get_elems(), lst -> get_elems(), func_name, return_type, symtab_map);
     }
     ;
...
procedure_call: 
     IDENTIFIER '(' ')' ';'
     {
          ...
          vec.push_back(iden);
          $$ = new class proccall_astnode(vec, $1);
     }
...
printf_call:
     ...
     {
          class identifier_astnode* iden = new class identifier_astnode($1);
          class string_astnode* str_lit = new class string_astnode($3);
          types = new class types_info("string");
          str_lit -> set_type_info(types);
          vector<class exp_astnode*> vec;
          vec.push_back(iden);
          vec.push_back(str_lit);
          $$ = new class proccall_astnode(vec, $1, $3);
     }
...
postfix_expression:
     ...
     | IDENTIFIER '(' ')'
     {
          ...
          vector<class exp_astnode*> vec;
          vec.push_back(iden);
          $$ = new class funcall_astnode(vec, $1);
          $$ -> set_type_info(types);
